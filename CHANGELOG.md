...
- **git 提交后自动刷新侧边栏工作区状态**（issue #90）：监听当前仓库 `.git` 目录的变化，本地提交（父仓库或子模块）后去抖自动重算侧边栏，无需手动刷新
...
- **branch 版本来源的模块卡片直接显示追踪的分支名**（issue #90）：不再追踪 / 展示 commit SHA，本地提交只推进子模块 HEAD、分支名不变，卡片展示不再过期；确认对话框与后台提交信息补全同步适配（branch 不再补全）
