...
- **固定版本子模块一致性恢复**（issue #96）：插件启动与刷新时校验 commit / tag / release 版本来源的 submodule 实际 HEAD 与配置 `ref` 是否一致，不一致时合并弹窗（modal，列出全部模块与 `当前SHA → 配置SHA`）给出**三个选项**：「跟随本地配置文件」（保持固定版本，先临时解锁，fetch 后 checkout 到配置记录的提交（可能联网），恢复后重新锁定）、「跟随 git submodule 版本」（保留本地 HEAD，切换为 branch 跟踪语义并更新配置记录，之后不再提示）、「取消」；弹窗通过本地只读分析（`git merge-base --is-ancestor`）为每个模块标注推荐——配置 commit 是本地 HEAD 的祖先（疑似本地 git 操作更新）推荐跟随 submodule，否则推荐跟随配置文件；取消后本会话不再重复弹窗，但**手动配置变更操作（apply / update / remove / lock / switch 等）后的刷新会重新检查**（用户在场可即时决策，本次检查忽略会话跳过集合）；检查 → 弹窗 → 执行全程有并发锁，启动 / git watcher / 手动刷新重叠时只弹一次；branch 版本来源仍跟随本地 HEAD（issue #90 反向同步），copy / release 附件方式无 git HEAD 概念不校验
- **刷新新增「根据本地 submodule 更新配置文件」模式**（issue #96）：刷新命令第三个选项，以本地为准（纯本地不联网）把未跟踪 submodule 加入配置、branch 条目同步实际 HEAD 与**本地分支名**、固定版本条目切换为 branch 跟踪并写回配置与版本缓存；branch 条目本地所在分支与配置 `versionRef` 不一致（HEAD 可能恰好相同）时同样会被检出并同步，反馈新增/跟随数量
...
- **侧边栏操作图标换用 VS Code 标准 codicon 图形**（issue #92）：更新 / 打开 GitHub / 打开 README / 创建仓库 / 打开目录 / 切换引入方式 / 锁定等全部按钮图标改为实心风格，小尺寸下辨识度更高，悬浮提示保留
