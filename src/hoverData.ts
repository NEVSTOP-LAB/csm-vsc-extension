...
const HOVER_DB: Record<string, HoverEntry> = {
    ...
    '=>': {
        ...
        detail: [
            ...
            '',
            '支持使用分号 `;` 分隔多个变量名以保存多个返回值（CSM 完整版）。',
            '',
            '**格式**',
            '```csm',
    ...
    '<INCLUDE': {
        ...
        detail: [
            '将另一个 `.csm` 文件的内容嵌入到当前脚本中执行，类似函数调用。',
            '',
            '**格式**',
            '```csm',
            '<include path/to/script.csm>',
            '```',
            '',
            '**示例**',
            '```csm',
            '<include samples/include-sequence.csm>',
            '```',
        ].join('\n'),
    },
...
// ---------------------------------------------------------------------------
// Standalone hover function (used by both CSM and CSMLog providers)
// ---------------------------------------------------------------------------

export function provideCSMHover(
    document: vscode.TextDocument,
    ...
    // because `-` is a word separator in CSM's wordPattern.
    if (ltPos !== -1) {
