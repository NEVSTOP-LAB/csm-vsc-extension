...
1. 分析用户提供的 Git diff 行或使用工具获取当前分支与 `main` 分支的差异，以收集所有已变更文件的信息。如果缺乏这些上下文，请要求用户提供。
2. 对照审查清单逐项检查
3. 发现不一致时，直接修复文档。如果代码逻辑过于复杂或缺乏针对最终用户的文案提示导致无法直接推断文档内容，请暂停修复并询问用户具体的功能描述，切勿自行捏造。如果对应的文档文件不存在，请先询问用户是否需要新建该文件。
4. 输出审查结果摘要
