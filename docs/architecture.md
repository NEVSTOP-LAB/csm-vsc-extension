...
**固定版本子模块一致性恢复**（issue #96）：commit / tag / release 版本来源的子模块——刷新时 `syncFixedVersionSubmodules` 逐个读取本地 HEAD（只读、不联网），与配置 `ref` 不一致时经 `analyzeSubmoduleDivergence`（`git merge-base --is-ancestor`，本地只读）分析分歧类型，合并弹窗（modal）列出全部模块（`当前SHA → 配置SHA`）并给出**三个选项**：「跟随本地配置文件」——保持固定版本，逐个 `restoreSubmoduleToRef`（先临时解锁 → `submodule update --init` → `fetch --tags origin`（可能联网）→ `checkout <ref>`（detached HEAD）→ 恢复锁定）；「跟随 git submodule 版本」——`followSubmoduleLocalHead` 将模块切换为 branch 跟踪语义（`versionKind: branch` + 当前分支名 + HEAD 写回配置与版本缓存），之后由 `syncTrackedSubmoduleVersions` 自动跟随本地，不再触发检查；「取消」。弹窗为每个模块标注推荐：配置 commit 是本地 HEAD 祖先（疑似本地 git 操作更新）推荐跟随 submodule，否则推荐跟随配置文件。取消后本会话跳过，但**手动配置变更命令**（apply / update / remove / switch method / lock / record / link / create / initialize）成功后的刷新以 `forceFixedVersionCheck` 忽略会话跳过集合重新检查（用户在场可即时决策）。检查 → 弹窗 → 执行全程由 `fixedVersionSyncInFlight` 并发锁保护，启动 / git watcher / 手动刷新重叠时只弹一次。branch 来源由 `syncTrackedSubmoduleVersions` 反向同步（跟随本地 HEAD），copy / release 附件方式无 git HEAD 概念不校验。触发时机：插件启动（`register()` 后台刷新）、所有现有刷新路径（手动刷新、git watcher 去抖刷新）与手动配置变更后。

**刷新第三模式——根据本地 submodule 更新配置文件**（issue #96）：`refreshCommand` 的 QuickPick 第三项 `sync-submodules`，调用 `syncConfigFromLocalSubmodules` 以本地为准（纯本地不联网）同步配置：`syncSubmoduleEntriesToConfig` 把未跟踪 submodule 加入配置 → `syncTrackedSubmoduleVersions` 同步 branch 条目实际 HEAD → 固定版本条目（HEAD ≠ ref）经 `followLocalForSubmodules` 切换为 branch 跟踪并写回配置与版本缓存，最后反馈新增/跟随数量。
