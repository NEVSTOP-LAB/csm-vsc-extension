...
- **更新模块时支持选择具体版本**（issue #37）：
  ...
  - **branch 版本来源直接显示追踪的分支名**（issue #90）：不追踪 / 不展示 commit SHA——本地提交只会推进子模块 HEAD，分支名不变，卡片展示不会过期；`branch` 类型不再重复展示「分支：xxx」徽章，确认对话框的「当前版本」同样显示分支名，且后台提交信息补全跳过 `branch` 条目
  - **git 提交后自动刷新**（issue #90）：扩展监听当前仓库 `.git` 目录的变化，在父仓库或子模块中提交代码后会自动重算侧边栏本地工作区状态（去抖），无需手动刷新
...
- **branch 版本的展示语义**（issue #90）：`versionKind: branch` 的模块以追踪的分支为准——卡片与确认对话框直接显示分支名，`ref` 仅作为应用/更新时的快照记录，不参与展示；在子模块内提交代码后展示自动保持最新，无需同步 commit SHA
