...
| 激活事件 | 触发条件 | 推荐场景 |
|---------|---------|---------|
| `onLanguage:csm` | 打开 CSM 文件时 | **语言插件首选** |
| `onCommand:csm.run` | 用户执行指定命令时 | 命令触发类功能 |
...
```jsonc
// syntaxes/csm.tmLanguage.json
{
  "$schema": "https://raw.githubusercontent.com/martinring/tmlanguage/master/tmlanguage.json",
  "name": "CSM",
  "scopeName": "source.csm",    // 顶层作用域，在 package.json grammars 中引用
  "patterns": [
    { "include": "#comments" },
    { "include": "#keywords" },
    { "include": "#strings" },
...
│   └── launch.json               # F5 调试配置
├── syntaxes/
│   └── csm.tmLanguage.json # TextMate Grammar
├── snippets/
│   └── csm.code-snippets   # 代码片段
├── icons/
│   ├── csm-light.svg       # 文件图标（浅色主题）
│   └── csm-dark.svg        # 文件图标（深色主题）
├── src/
