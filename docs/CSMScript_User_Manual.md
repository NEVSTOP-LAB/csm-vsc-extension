...
本手册包含 CSMScript 脚本执行引擎的完整功能文档，涵盖所有已完成的功能模块。当前版本：**0.3.2**


## 目录

- [第一章 概述](#第一章-概述)
  - [1.1 CSMScript 简介](#11-csmscript-简介)
  - [1.2 版本对比](#12-版本对比)
  - [1.3 操作系统支持](#13-操作系统支持)
- [第二章 CSM 接口](#第二章-csm-接口)
  - [2.1 CSM Engine](#21-csm-engine-enginecsmvi)
  - [2.2 CSM 界面组件](#22-csm-界面组件)
  - [2.3 CSMScriptApp](#23-csmscriptapp-csmscriptappvi)
- [第三章 执行引擎](#第三章-执行引擎)
  - [3.1 脚本结构](#31-脚本结构)
  - [3.2 基础功能](#32-基础功能)
...
CSMScript 是一款基于 Communicable State Machine (CSM) 框架的脚本执行引擎，旨在提供强大的脚本执行能力，支持复杂的测试逻辑和流程控制。通过 CSMScript，用户可以编写灵活的测试脚本，实现自动化测试。

CSMScript 分为两个版本：

- **CSMScript-Lite**: 提供基础的脚本执行功能，适用于简单的测试需求，开源且免费使用。
- **CSMScript**: 提供完整的脚本执行功能，支持复杂的逻辑控制和扩展功能，适用于高级测试需求，需获取授权使用。

## 1.2 版本对比

| 功能说明             | 具体说明                                     | CSMScript-Lite | CSMScript |
| -------------------- | -------------------------------------------- | ------------- | -------- |
| 脚本执行功能         |                                              | ✅            | ✅       |
...
发布版本的 CSMScript 主要支持 **Windows** 操作系统。大部分使用场景均基于 Windows，且授权（license）功能依赖 Windows 相关的系统 API。

> [!NOTE]
>
> 如需在 **Linux** 或 **NI RT** 等系统上使用 CSMScript，请联系 [NEVSTOP-LAB](https://github.com/NEVSTOP-LAB) 获取定制支持。
...
CSM Engine 是 CSMScript 的核心组件，负责解析和执行 CSM 脚本。它提供了一个强大的脚本执行环境，支持各种功能和特性，使得 CSM 脚本能够高效地完成测试任务。

### 消息(Message)

| 消息(Message)            | 说明                                   | 参数                                         | 返回值                                       |
| ------------------------ | -------------------------------------- | -------------------------------------------- | -------------------------------------------- |
| "TS: Load Sequence File" | 载入csmscript 文件                     | 文件路径，可以使用相对路径                   | N/A                                          |
| "TS: Load Sequence"      | 载入csmscript 脚本                     | 脚本字符串，格式支持 MASSDATA,SAFESTR,HEXSTR | N/A                                          |
...
| 广播(Broadcast)          | 说明                  | 参数                                         |
| ------------------------ | --------------------- | -------------------------------------------- |
| "SequenceLoaded Event"   | csmscript 脚本被载入  | 脚本字符串，格式支持 MASSDATA,SAFESTR,HEXSTR |
| "SequenceUnloaded Event" | csmscript 脚本被卸载  | N/A                                          |
...
## 2.3 CSMScriptApp (`CSMScriptApp.vi`)

CSMScriptApp 是 CSMScript 的主程序，在 Engine 组件和 UI 组件基础上构建。

![CSMScriptApp](./_image/csmscript%20ui.png)
...
| "API: Load Script File"     | 弹出对话框，选择csmscript文件并载入 | N/A  | N/A    |
| "API: Unload Sequence File" | 卸载csmscript文件                   | N/A  | N/A    |
...
> [!NOTE]
> 配置项指的是 CSM 原生的 INI 配置支持，假如 CSMScriptApp 实例化的 csm 模块名为 app, 那么配置项类似于：
>
> ```ini
> [app]
> EnableDebugWindow=TRUE
> DefaultScriptFile=C:\path\to\script.csmscript
> ```
...
CSMScript 脚本由两部分组成：**预定义区域（Pre-definition Section）** 和 **脚本区域（Script Section）**。
...
脚本区域是 CSMScript 的主体部分，紧随预定义区域之后，包含所有要执行的脚本指令。脚本区域使用 CSM 消息语法（参见[CSM 基本语法](#32-基础功能)）以及 CSMScript 扩充指令集（参见[扩充的指令集](#35-扩充的指令集)）编写。
...
CSMScript 基于 CSM 框架的消息语法，在脚本中描述对各 CSM 模块的调用和交互。
...
CSM/CSMScript 默认只支持 STRING 作为参数（继承自 JKISM），但实际应用中需要传输的数据种类繁多。下表列出了当前支持不同数据类型的几种方式，其中部分是 CSM 内置的，其他则需要安装额外的支持插件。
...
脚本运行过程中，CSMScript 会维护一个临时变量空间，用于存储脚本执行过程中产生的变量值。
...
- **TAGDB_VAR_SPACE_NAME** 用于指定 TagDB 变量空间名称，参数支持数组，默认为 "csmscript"
...
CSMScript 内置了一些非 CSM 提供的扩充指令，用于实现一些基本的功能。例如：goto 指令等。格式为 `指令 >> 参数`。

CSMScript 还提供了扩展指令集的方案，用户可以根据需要定义新的指令集接口，映射到已有模块的指令集上，从而实现新的指令集功能。

| feature           | 说明                                         | CSMScript-lite | CSMScript |
| ----------------- | -------------------------------------------- | ------------- | -------- |
...
| 指令                           | 说明                                                                      | CSMScript-lite | CSMScript |
| ------------------------------ | ------------------------------------------------------------------------- | ------------- | -------- |
...
用于指定 TagDB 变量空间名称，参数支持数组，默认为 "csmscript"。
...
用于在脚本执行界面输出指定的信息，方便用户调试脚本或查看参数状态，给参数赋值。ECHO 指令会同时广播一个同名的广播，参数为 ECHO 指令的输出内容，方便订阅该广播进行处理。可以订阅 ECHO 广播，将输出内容保存到日志文件中。CSMScript UI 提供了一个 [EchoView](#42-echo信息窗口-echoviewcsmvi) 显示窗口供使用。
...
CSMScript Script 分为两部分： 预定义区域（predefined section）和脚本区域（script section）。
...
分支逻辑的支持是脚本中的重要功能之一。CSMScript 的分支逻辑支持通过[表达式支持](#36-表达式支持)中支持的表达式来进行条件判断，从而实现不同条件下执行不同指令的功能。

| 功能                | 说明           | CSMScript-Lite | CSMScript |
| ------------------- | -------------- | ------------- | -------- |
...
循环支持是脚本中的重要功能之一。CSMScript 的循环支持通过[表达式支持](#36-表达式支持)中支持的表达式来进行条件判断，从而实现重复执行指令的功能。可以在循环体内使用 [BREAK 和 CONTINUE](#break-指令) 指令控制循环流程。
...
文件路径可以是全路径或相对路径。相对路径时，路径是相对于当前脚本文件所在目录，如果时内存加载的脚本，则相对于 CSMScript 的工作目录。
...
```c
// 定义一组通用的脚本逻辑，保存为 SEQ-Hardware-startup.csmscript 文件
initialize >> daq1 -@ ai
initialize >> daq2 -@ ao
...
```c
<include SEQ-Hardware-startup.csmscript>
```
...
- **CSMScript-Lite**: 提供基础的脚本执行功能，适用于简单的测试需求，可免费使用。
- **CSMScript**: 提供完整的脚本执行功能，支持复杂的逻辑控制和扩展功能，适用于高级测试需求，需获取授权使用。
...
CSMScript 采用离线认证的方式进行授权。本地根据机器信息生成原始证书码，用户将原始证书码发送给授权方进行验证。授权方对原始证书码进行签名，并以 `.lic` 文件格式或证书信息的形式返回给用户，用于激活软件。
...
> [!NOTE]
>
> 未激活的 CSMScript 将以试用模式运行，license 相关行为：
>
> - 10 分钟后，将向所有 CSM 模块发送 "Macro: Exit" 命令，退出全部模块
> - 所有 script 载入后，自动在脚本后加入 ECHO 试用版本的相关信息
> - CSMScriptAPP 的 Title 上将显示 "CSMScript (Un-Licensed Version)" 的字样
...
本文档以一个 **IoT 电源管理板 (Power Management Board)** 自动化测试场景为例，演示如何使用 CSMScript 构建完整的自动化测试系统，涵盖从需求分析、模块设计、流程编排到脚本实现的全流程。
...
本测试系统由 4 个 CSM 后台模块构成，各模块通过 CSM 消息总线通信，CSMScript 脚本作为测试流程的"指挥官"，统一调度各模块执行操作。
...
### 5.1 硬件初始化子脚本 `SEQ-PCB-Init.csmscript`

```c
// ============================================================
// SEQ-PCB-Init.csmscript
...
### 5.2 硬件关闭子脚本 `SEQ-PCB-Close.csmscript`

```c
// ============================================================
// SEQ-PCB-Close.csmscript
...
### 5.3 主测试脚本 `SEQ-PCB-Main.csmscript`

```c
// ============================================================
// SEQ-PCB-Main.csmscript
// PCB 电路板自动化测试 - 主控脚本
// ============================================================
...
<include SEQ-PCB-Close.csmscript>
echo >> ============================================================
echo >> 测试完成
echo >> ============================================================
```
...
在此 PCB 自动化测试场景中，CSMScript 具有以下优势：

| CSMScript 优势 | 说明 |
| -------------- | ---- |
| **流程修改灵活** | 直接编辑 `.csmscript` 文本文件，无需重新编译部署 |
...
下图展示了本测试系统中各 CSM 模块之间的通信关系。CSMScript 引擎作为中央调度者，通过 CSM 消息总线与各仪器驱动模块通信。
...
graph LR
    ...
    subgraph EngineGrp["CSMScript 引擎"]
        Eng["Engine(CSM).vi"]
...
通过 CSMScript 的变量空间功能，可以在启动时注入测试配置，无需修改脚本内容：
...
本文档以一个 **锂电池管理系统（BMS）总线功能验证测试** 场景为例，演示如何使用 CSMScript 构建 CAN/AI 多源同步采集的自动化测试系统，涵盖从需求分析、模块设计、两级数据整合到离线分析报表的完整流程。
...
本测试系统由 **6 个 CSM 后台模块** 构成，分为采集层、整合层和存储分析层。各模块通过 CSM 消息和广播订阅相互连接，形成自驱动的数据处理管线；CSMScript 脚本作为统一调度者，只负责配置、启动和等待。
...
### 5.1 硬件初始化子脚本 `SEQ-BUS-Init.csmscript`

```c
// ============================================================
// SEQ-BUS-Init.csmscript
// 硬件初始化公共脚本 - 被主脚本通过 <include> 引用
// ============================================================
...
### 5.2 硬件关闭子脚本 `SEQ-BUS-Close.csmscript`

```c
// ============================================================
// SEQ-BUS-Close.csmscript
// 硬件资源释放公共脚本 - 被主脚本通过 <include> 引用
// ============================================================
...
### 5.3 主测试脚本 `SEQ-BUS-Main.csmscript`

```c
// ============================================================
// SEQ-BUS-Main.csmscript
// BMS 总线功能验证测试 - 主控脚本
//
...
// ============================================================
echo >> [MAIN] 阶段 0：硬件初始化
<include SEQ-BUS-Init.csmscript>
...
// ============================================================
<done>
<include SEQ-BUS-Close.csmscript>
echo >> ============================================================
echo >> 测试完成
echo >> ============================================================
```
...
在 CAN/AI 多源同步采集的总线测试场景中，CSMScript 具有以下优势：
...
| **版本管理友好** | `.csmscript` 纯文本，Git diff 清晰，Code Review 友好 |
...
通过 CSMScript 的变量空间功能，从 INI 文件读取硬件资源和路径配置，无需修改脚本：
...
本文档以一个 **工业离心泵出厂检验** 场景为例，演示如何在**围绕实时状态（全局 TagDB）进行模块设计**的架构中使用 CSMScript 管理多个独立的测试场景。各功能模块围绕**全局 TagDB 内存缓存**设计（基于 [LabVIEW-TagDB](https://github.com/NEVSTOP-LAB/LabVIEW-TagDB)）：VirtualPump 模块通过 TCP 与下位机通信并持续更新全局 TagDB，MonitorUI 和 TDMSManager 等功能模块直接读取 TagDB 中的数据，无需通过广播互相沟通；CSMScript Engine 内置了一套 **TAGDB 系列指令**（`TAGDB_WAIT_FOR_EXPRESSION`、`TAGDB_START_MONITOR_EXPRESSION`、`TAGDB_SWEEP` 等），使脚本可以直接以 TagDB 中的实时物理量为条件进行流程控制——这也是专门设计本场景的原因。
...
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  CSMScript 引擎  |  SEQ-PUMP-SC01/SC02/SC03.csmscript    │  │
...
**TagDB 是整个上位机的全局共享内存**，不是一个独立的 CSM 消息处理模块。VirtualPump、TDMSManager、MonitorUI 以及 CSMScript Engine 都直接访问同一个 TagDB，无需经过消息路由。
...
### 5.1 SC01 密封耐压测试 `SEQ-PUMP-SC01.csmscript`
...
```c
// ============================================================
// SEQ-PUMP-SC01.csmscript - 密封耐压测试（可单独运行）
// 分步升压 → 保压 5 分钟 → 压降判定
// ============================================================
...
### 5.2 SC02 脉冲冲击测试 `SEQ-PUMP-SC02.csmscript`
...
```c
// ============================================================
// SEQ-PUMP-SC02.csmscript - 脉冲冲击测试（可单独运行）
...
### 5.3 SC03 额定工况性能测试 `SEQ-PUMP-SC03.csmscript`
...
```c
// ============================================================
// SEQ-PUMP-SC03.csmscript - 额定工况性能测试（可单独运行）
...
下图展示各功能模块之间的关系。**全局 TagDB** 是核心的共享内存缓存，VirtualPump 持续写入（从下位机读取数据 + 监控脚本写入的设定点），MonitorUI、TDMSManager 和 CSMScript Engine 直接读取。功能模块之间不通过广播互相通信。
...
graph LR
    ...
    Script["CSMScript 脚本<br/>TAGDB_SWEEP<br/>TAGDB_SET_VALUE<br/>TAGDB_WAIT_FOR_EXPRESSION<br/>TAGDB_START_MONITOR_EXPRESSION"]
    MUI["MonitorUI<br/>自主刷新"]
    CSM["CSMScript Engine<br/>流程控制"]
...
利用 CSMScript 内置的 `INPUT_DIALOG` 指令，让测试人员在测试开始前输入泵型号、批次和操作员信息，再通过 [CSM-INI-Static-Variable-Support](https://github.com/NEVSTOP-LAB/CSM-INI-Static-Variable-Support?tab=readme-ov-file#nested-variables) 的**嵌套变量**功能，将上述输入自动拼接成本次测试专属的 TDMS 文件路径：
...
| 0.3.1   | 2026 | 新增脚本结构章节，介绍预定义区域与脚本区域；新增配置覆盖逻辑说明；更新各章节中有关预定义区域初始化的 NOTE 说明；新增 TS: Highlight Exec 消息说明；补充界面组件广播表；新增 CSMScriptApp 截图；修正 3.6 节函数表格（移除不可用的 rnd 函数，新增 WARNING 说明）；修正 EnableDebugWindow 配置项描述；新增授权说明 OS 支持说明（仅支持 Windows，Linux/NI RT 需定制） |
| 0.3.0   | 2026 | 将项目名称从 CSMStand 更名为 CSMScript；同步更新所有文档引用；新增 CSMScriptApp 配置项章节；更新泵出厂测试章节标题；修正案例章节 mermaid 流程图换行符格式 |
...
[📖 Wiki Home](https://github.com/NEVSTOP-LAB/CSMScript/wiki) | [🏠 CSMScript Repo](https://github.com/NEVSTOP-LAB/CSMScript) | [🏢 NEVSTOP-LAB](https://github.com/NEVSTOP-LAB) | [⚙️ CSM Framework](https://github.com/NEVSTOP-LAB/Communicable-State-Machine) | [📚 CSM Wiki](https://github.com/NEVSTOP-LAB/Communicable-State-Machine/wiki) | [🔑 授权说明](https://github.com/NEVSTOP-LAB/CSMScript/wiki/engine-license)

_Powered by CSM, NEVSTOP-LAB_

_CSMScript 用户手册 - 版本 0.3.2_
