...
3. [语言插件推荐配置（CSM 示例）](#3-语言插件推荐配置csm-示例)
...
> 如只关注 CSM 语言插件的配置，可直接跳至 [第 3 节](#3-语言插件推荐配置csm-示例)。
...
对于 **新脚本语言支持类插件**（如 CSM），绝大多数功能仅需以下几个激活事件的组合：

```jsonc
// package.json（CSM 语言插件典型配置）
{
  "activationEvents": [
    // 1. 打开 CSM 文件时激活（语法高亮、补全、诊断等）
    //    VSCode 1.74+ 中，若 contributes.languages 已声明该语言，此行可省略
    "onLanguage:csm",

    // 2. 工作区包含 CSM 特征文件时激活（项目级功能）
...
| 代码补全 / 悬停 / 诊断（需要代码逻辑） | `onLanguage:csm` | 文件打开时激活，初始化各 Provider |
| 语言服务器（LSP） | `onLanguage:csm` 或 `workspaceContains:**/*.csms` | 按场景选择；项目级服务器推荐 `workspaceContains` |
| 自定义命令（格式化、运行脚本等） | 无需声明（1.74.0+）或 `onCommand:csm.*` | 插件贡献的命令会自动触发激活 |
| 自定义侧边栏视图 | 无需声明（1.74.0+）或 `onView:<viewId>` | 插件贡献的视图会自动触发激活 |
| 调试支持 | `onDebug` / `onDebugInitialConfigurations` / `onDebugResolve:csm` | 根据调试功能复杂度选择，见[调试系列详解](#ondebug-系列) |
| 任务提供者（Run/Build Task） | 无需声明（1.76.0+）或 `onTaskType:csm` | 插件贡献的任务类型会自动触发激活 |
...
**语言扩展场景：** 适合需要在项目层面初始化的功能（如语言服务器），打开包含 CSM 文件的工程目录时即自动启动。
...
```json
"activationEvents": [
  "onDebugAdapterProtocolTracker:csm"
]
```
